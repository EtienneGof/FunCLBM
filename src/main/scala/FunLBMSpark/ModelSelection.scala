...
object ModelSelection {
  ...
  def gridSearch(data: RDD[(Int, Array[DenseVector[Double]])],
    ...
    val allRes = gridRange.map(KL => {
      ...
      val res = latentBlock.run(data,EMMethod, nConcurrent=nConcurrentEachTest, nTryMaxPerConcurrent = nTryMaxPerConcurrent, verbose=verbose)
  ...
  def greedySearch(data: RDD[(Int, Array[DenseVector[Double]])],
    ...
    val prevLB = new FunLBMSpark.LatentBlock().setK(1).setL(1)
    ...
    val prevModel = prevRes("Model").asInstanceOf[LatentBlockModel]
    var prevIcl = prevRes("ICL").asInstanceOf[List[Double]].last

    val tmpLB = new FunLBMSpark.LatentBlock().setMaxIterations(5).setMaxBurninIterations(5).setUpdateLoadingStrategy(4)

    if(verbose){println("Initial Icl: "+prevIcl.toString)}

    @tailrec def go(prevRes: Map[String,Product], prevModel: LatentBlockModel, prevIcl: Double): Map[String,Product] = {
      ...
      val candidateModelK = resCandidateK("Model").asInstanceOf[LatentBlockModel]
      val candidateModelL= resCandidateL("Model").asInstanceOf[LatentBlockModel]
